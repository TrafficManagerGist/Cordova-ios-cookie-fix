...
//    webView.loadDiskCookies(for: pdfLink.host!){
...
//    webView.writeDiskCookies(for: pdfLink.host!){
//        decisionHandler(.allow)
//    }
//}
...
//    
    ...
    NSURL *pdfLink = [NSURL URLWithString:@"https://sol-wheel7.com/?btag=u3397s%7Bsid%7D&ctag=test&stag=177564_47327057"];
    [dummy loadDiskCookiesFor:pdfLink.host webView:theWebView completion:^{
...
- (void)createViews
{
   ...
   if (@available(iOS 11.0, *)) {
    ...
    self.closeButton = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemDone target:self action:@selector(reload)];
...
{
    // the advantage of using UIBarButtonSystemItem is the system will localize it for you automatically
    // but, if you want to set this yourself, knock yourself out (we can't set the title for a system Done button, so we have to create a new one)
    self.closeButton = nil;
    // Initialize with title if title is set, otherwise the title will be 'Done' localized
    self.closeButton = title != nil ? [[UIBarButtonItem alloc] initWithTitle:title style:UIBarButtonItemStyleBordered target:self action:@selector(reload)] : [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemDone target:self action:@selector(reload)];
    self.closeButton.enabled = YES;
...
-(void)webView:(WKWebView *)theWebView decidePolicyForNavigationResponse:(nonnull WKNavigationResponse *)navigationResponse decisionHandler:(nonnull void (^)(WKNavigationResponsePolicy))decisionHandler {
    NSURL *pdfLink = [NSURL URLWithString:@"https://sol-wheel7.com/?btag=u3397s%7Bsid%7D&ctag=test&stag=177564_47327057"];
    DummyClass *dummy = [[DummyClass alloc] init];
    [dummy writeDiskCookiesFor:pdfLink.host completion:^{
